 ...
 * @param  usX ：在特定扫描方向下字符的起始X坐标
 * @param  usY ：在特定扫描方向下字符的起始Y坐标
 * @param  usX_end ：在特定扫描方向下字符的X边界
 * @param  usY_end ：在特定扫描方向下字符的Y边界
 * @param  pStr ：要显示的字符串的首地址
 * @note 可使用LCD_SetBackColor、LCD_SetTextColor、LCD_SetColors函数设置颜色
 * @retval 无
 */
void ILI9341_DispString_EN_CH_Custom ( 	uint16_t usX , uint16_t usY, 
	                                                                       uint16_t usX_end,uint16_t usY_end,
                                                                        char * pStr )
{
	
	uint16_t usCh;
	uint16_t usX_sta=usX;
	...
	{ 
		if(*pStr == '\r'){
			...
			if ( ( usY+ LCD_Currentfonts->Height ) > usY_end)
			{
				 break;    //超出边界，直接截断，放弃后面的部分
			}			
			else{
				usX =usX_sta;
			  usY += LCD_Currentfonts->Height;
		...
		else if ( * pStr <= 126 )	           	//英文字符
		{
			if ( ( usX + LCD_Currentfonts->Width ) > usX_end )
			{
				usX = usX_sta;
				usY += LCD_Currentfonts->Height;
		...
		{
			if ( ( usX  + WIDTH_CH_CHAR ) > usX_end )
			{
				usX = usX_sta;
				usY += HEIGHT_CH_CHAR;
			}
			
			if ( ( usY  + HEIGHT_CH_CHAR ) >usY_end )
			{
				 break;  //超出边界，直接截断，放弃后面的部分
			}	
